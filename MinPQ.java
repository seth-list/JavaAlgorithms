...
public class MinPQ<Item extends Comparable<Item>> implements Iterable<Item>
{
   ...
   private void swim(int number)
   {
      ...
      while(number > 0 && greater((number - 1)/2, number))
      {
   ...
   private void sink(int number)
   {
      ...
      while(2*(number + 1) <= N)
      {
        ...
        if (j + 1 < N && greater(j, j+1)) //если существует второй дочерний элемент, это означает что j < N
   ...
   public Item delMin()                    // remove and return a min item
   { 
   ...
   { 
     boolean tr = a[i].compareTo(a[j]) > 0;
     
     //System.out.print("what!!!");
     //System.out.print(a[i] + " > " + a[j] + " is " + tr);
  ...
  { 
     ...
     Key element = (Key)d.delMin();
     System.out.println (element.getKey());
   
     element = (Key)d.delMin();
     System.out.println (element.getKey());
